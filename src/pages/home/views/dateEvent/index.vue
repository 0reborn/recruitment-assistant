<template>
   <div class="day_event">
      ...
      <div class="main">
         <div v-if="event.status && event.status.length != 0"
            class="statusInfo">
            <div class="info">
               <div class="left">
         ...
         <div v-else>
            <a @click="addStatus"
               href="#"
               class="addStatusBtn btn btn-sm btn-info">添加状态</a>
         </div>

         <div class="elseInfo"
            ref="elseInfo">
            <div v-for="prop in event.pros"
               :key="prop.key"
               class="info">
            ...
            <a v-if="event.status && event.status.length != 0"
               @click="editRecord"
               href="#"
               class="updateRecordBtn btn btn-sm btn-info">更新状态</a>
         </div>

      </div>
      <div @click="unfold"
         class="unfold-btn">
         <div v-if="fold">更多<i class="el-icon-caret-bottom"></i></div>
         <div v-else-if="!fold">收起<i class="el-icon-caret-top"></i></div>
      </div>
   </div>
</template>
...
export default {
   ...
   methods: {
      ...
      addStatus () {
         this.$root.$children[0].$refs.addStatusFrame.show(this.event.rid)
      },
      editRecord () {

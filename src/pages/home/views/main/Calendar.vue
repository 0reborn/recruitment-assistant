  ...
  <div id="calendar">
    ...
    <div class="week-box container-fluid">
      <div class="week-box-header col-12">
        <button @click="lastWeek" class="btn btn-info">上一周</button>
        <button @click="unfold" class="btn btn-info">全部日历</button>
        <button @click="nextWeek" class="btn btn-info">下一周</button>
      </div>
      <div class="week-box-main col-12">
        <div class="date" v-for="date in LatterWeekDates" :key="date[0]">
          <div class="date-header">{{ date[0] }}&nbsp;&nbsp;&nbsp;{{ date[1] }}</div>
          <div class="date-event-box">
            <div v-if="date[2]">
              <DateEvent
                v-for="event in sortByRemainDayNum(dateEvent[date[0]])"
                :key="event.rid"
                class="date-event"
                :event="event"
              ></DateEvent>
